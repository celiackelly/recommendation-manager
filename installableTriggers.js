...
function createTriggers() {
  ...
  ScriptApp
  .newTrigger('addUuidAndEmailCheckbox')
  .forSpreadsheet(ss)
  .onFormSubmit()
  .create();
...
function sortSheetsAlphabetically () {
  ...
  sheetNames.sort((a, b) => {
    const sortOrder = ['Form Responses 1', 'Due Dates', 'Address Book', 'Template']   
...
function createNewSheetsOnSubmit(e) {
  ...
  const teacherCellValues = formResponsesSheet.getRange(`${formResponses.columnLetters.mathTeacher}${newRow}:${formResponses.columnLetters.supplementalTeacher}${newRow}`).getValues()[0]   //[mathTeacherCell, laTeacherCell, supplementalTeacherCell]
  ...
  teacherNames.forEach((name, i) => {
    ...
    if (name !== null && !sheetNames.includes(name)) {
      ...
      const sheetId = ss.insertSheet(name, sheetsArray.length, {template: templateSheet}).getSheetId()
    ...
    "warningOnly": false, 
    "editors": {
      "users": ["ckelly@nysmithschool.com", "bschrembs@nysmithschool.com", `${name}@nysmithschool.com`], 
      "domainUsersCanEdit": false
    }
  }
...
function addUuidAndEmailCheckbox(e) {      
  ...
  formResponsesSheet.getRange(newRow, formResponses.columnNumbers.uuId).setValue(Utilities.getUuid())
  ...
  formResponsesSheet.getRange(newRow, formResponses.columnNumbers.queueEmails).setDataValidation(rule);
  ...
  formResponsesSheet.getRange(newRow, formResponses.columnNumbers.primaryContactEmail).setFormula(`=iferror(query('Address Book'!A2:E, "SELECT C, E WHERE A='${studentName}' AND A IS NOT NULL", 0), "")`)

  //get the cell ranges for each teacher submission 
  const mathTeacherCell = formResponsesSheet.getRange(newRow, formResponses.columnNumbers.mathTeacher)  
  const laTeacherCell = formResponsesSheet.getRange(newRow, formResponses.columnNumbers.laTeacher)            
  const supplementalTeacherCell = formResponsesSheet.getRange(newRow, formResponses.columnNumbers.supplementalTeacher)  
  
  //array of submitted teacher cells (ranges, not values)
  const teacherCells = [mathTeacherCell, laTeacherCell, supplementalTeacherCell]
...
function markCompletion(e) {
  ...
  if (range.getColumn() !== 6 || currentSheet.getName() === 'Form Responses 1') { 
  ...
  const teacherNameCells = formResponsesSheet.getRange(responseRow, formResponses.columnNumbers.mathTeacher, 1, 3)  
  ...
  } else {  //if date completed is deleted, reset background on 'Form Responses 1' page and teacher sheet
